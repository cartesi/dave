...
//! docs/plans/measurements.md. Run through `just measure`; committing a
...
fn main() -> Result<()> {
    ...
    let mut report = String::new();
    preamble(&mut report, &image, args.full)?;
...
fn preamble(report: &mut String, image: &Path, full: bool) -> Result<()> {
...
// docs/plans/node-refactor.md, superseding prt/measure_constants.
// Fixes that port carries (docs/dimensioning.md, measurement
// discipline): halt AND yield guarded on every timed region,
// steady-state input-fed sampling instead of boot, conservative
// floor rounding instead of floor+1.
//
...
fn constants_report(
    ...
    writeln!(
        report,
        "Constants changes are contract changes; time any bump against\n\
