...
//! Rust mirror of `cartesi::machine_runtime_config` from the v0.21 cartesi-machine
...
//!    v0.21 shape so silent drift is impossible.
...
impl Default for ConsoleRuntimeConfig {
    ...
    /// `os::TTY_DEFAULT_*` constants from v0.21 (`os.h`: cols=80, rows=25).
    fn default() -> Self {
        Self {
...
/// `skip_root_hash_store` fields. None of those exist on the v0.21
...
mod tests {
    ...
    /// Static schema-completeness test: pins the v0.21 `machine_runtime_config`
    ...
    fn test_runtime_config_schema_stability() {
        ...
        let typed: RuntimeConfig = serde_json::from_value(v021_json.clone())
            .expect("v0.21 runtime JSON should parse into RuntimeConfig");
        let reserialized = serde_json::to_value(&typed).expect("re-serialization failed");

        assert_eq!(
            v021_json, reserialized,
