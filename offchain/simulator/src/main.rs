...
pub async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let dispute_root_hash =
        dispute_root_hash(machine_factory.clone(), &simple_linux_program).await?;
...
fn create_player_arena(
    web3_private_key: String,
) -> Result<Arc<EthersArena>, Box<dyn std::error::Error>> {
